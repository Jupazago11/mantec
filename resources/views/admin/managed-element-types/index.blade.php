...
@section('title', 'Tipos de activos - Plantilla')
@section('header_title', 'Tipos de activos - Plantilla')
...
@section('content')
    <div class="space-y-8">
        <div>
            ...
            <p class="mt-2 text-slate-600">
                Crea y administra tipos de activos para los clientes que tienes asignados.
            </p>
        </div>
            ...
            <div class="xl:col-span-1">
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Registra un nuevo tipo de activo para uno de tus clientes.
                    </p>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-200 px-6 py-4">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            ...
                            <form method="GET" action="{{ route('admin.managed-element-types.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-end">
                                <div>
                        ...
                        <table class="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody class="divide-y divide-slate-200 bg-white">
                                @forelse($elementTypes as $elementType)
                                    @php
                                        $hasDependencies = ($elementType->components_count + $elementType->elements_count) > 0;
                                    @endphp
                                    ...
                                    <tr class="hover:bg-slate-50">
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-700">
                                            {{ $elementType->components_count }}
                                        </td>

                                        <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-700">
                                            {{ $elementType->elements_count }}
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-right">
                                            <div class="flex justify-end gap-2">
                                                <button
                                                    ...
                                                    data-description="{{ $elementType->description }}"
                                                    data-client_id="{{ $elementType->client_id }}"
                                                    data-action="{{ route('admin.managed-element-types.update', $elementType) }}"
                                                    onclick="openEditElementTypeModal(this)"
                                                >
                                                    Editar
                                                </button>
                                                ...
                                                @else
                                                    <form method="POST" action="{{ route('admin.managed-element-types.toggle-status', $elementType) }}">
                                                        @csrf
                                                        @method('PATCH')
    ...
    <script>
        ...
        function openEditElementTypeModal(btn) {
            document.getElementById('editElementTypeForm').action = btn.dataset.action;
            document.getElementById('edit_element_type_name').value = btn.dataset.name ?? '';
            document.getElementById('edit_element_type_description').value = btn.dataset.description ?? '';
