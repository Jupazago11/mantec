@extends('layouts.personal-guest')

@section('title', 'Iniciar sesión')

@section('content')
<div class="mx-auto flex min-h-screen max-w-5xl items-center justify-center px-4 py-6">
    <div class="grid w-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl lg:grid-cols-2">
        ...
        <div class="flex items-center justify-center p-6 sm:p-10">
            <div class="w-full max-w-md">
                ...
                @error('login')
                    <div class="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm text-red-700">
                        {{ $message }}
                    </div>
                @enderror

                <form method="POST" action="{{ route('personal.login.attempt') }}" class="space-y-5">
                    @csrf
                    <div>
                        <label for="username" class="mb-2 block text-sm font-medium text-slate-700">Usuario</label>
                        <input
                            type="text" name="username" id="username" value="{{ old('username') }}" autofocus
                            class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-[#d55b20] focus:ring-1 focus:ring-[#d55b20]"
                        >
                    </div>
