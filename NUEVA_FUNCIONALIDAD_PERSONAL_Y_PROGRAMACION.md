...
### 14.31 Historial De Comentarios Por Trabajador/Actividad — Responsable (App) + Administrativo (Bitácora) (2026-09-22)

**Pedido**: el supervisor, desde la app Android, puede comentar sobre las horas reportadas de UN trabajador puntual de una actividad (ej. "se fue temprano"), adicional al comentario general de la actividad. En Bitácora (web) esto se ve mezclado con los comentarios que ya ponían los administrativos, como un **historial**: nombre de quien comentó, y el texto — a futuro puede haber varios comentarios de administrativo, pero como máximo 1 del responsable por actividad+trabajador (editable/borrable desde la app). El campo de comentario de administrativo que antes se sobrescribía en cada guardado también pasa a ser historial (decisión confirmada con el usuario).

**Diseño de datos**: tabla nueva `activity_employee_comments` (`employee_id`, `date`, `activity_id` nullable — `null` = comentario de administrativo a nivel de día, no nulo = comentario del responsable sobre esa actividad puntual —, `author_employee_id` nullable, `author_name`, `comment`). Migración de datos aparte copia `bitacora_entries.comment` existentes hacia la tabla nueva (`activity_id = null`) y dropea esa columna — `bitacora_entries.corrected_value` no se toca. Modelo `App\Models\ActivityEmployeeComment` + relación `Activity::employeeComments()`.

**Backend API** (`Api\Personal\ActivityController`, mismo endpoint `POST /api/personal/actividades/{id}`, sin ruta nueva): `personas.*.comment` nuevo campo validado (`nullable|string|max:1000`). En `store()`, además del upsert de horas por persona, upsert (`ActivityEmployeeComment::updateOrCreate` por `activity_id`+`employee_id`) si el comentario no viene vacío, o `delete()` si viene vacío — a propósito **no** se auto-borran los comentarios existentes al marcar "Sí, todos trabajaron" por error (son texto escrito a mano, más "caro" de perder que un stepper de horas). `serialize()` expone `personas[].comment` (eager-load `employeeComments`).

**Backend Bitácora web** (`Personal\BitacoraController`): `index()` arma `$celdas[...]['comentarios']` (array, reemplaza el antiguo campo singular `comentario`) combinando `ActivityEmployeeComment` de ambos orígenes (`es_responsable` = `activity_id !== null`), ordenados por `created_at`. `saveEntry()` ya no hace upsert del comentario de admin — siempre `create()` (historial que se acumula), mientras que `corrected_value` sigue siendo upsert normal sobre `bitacora_entries`. Vista (`bitacora/index.blade.php`): tooltip y modal muestran la lista completa (autor + texto + etiqueta "(responsable)" cuando aplica), el textarea del modal ahora es "agregar un comentario nuevo" (arranca vacío, no pre-lleno con el valor guardado), con el historial ya existente mostrado en modo solo-lectura encima.

**App Android**: Room v20→21 (`MIGRATION_20_21`, `ALTER TABLE supervisor_personas ADD COLUMN comentario TEXT` — sin `DEFAULT` explícito, para coincidir exactamente con lo que exporta `app/schemas/.../21.json`, que es lo que Room valida al abrir la base). DTOs/entidad/DAO/repos/ViewModel con el campo `comentario` de punta a punta (`PersonalPersonaDto`, `SupervisorPersonaEntity`, `SupervisorPersonaDao.updateComentario`, `PersonalActivityLocalRepository.guardarComentarioPersonaLocal`, `SupervisorSyncRepository` push/refresh, `SupervisorActivityViewModel.guardarComentarioPersona`). UI (`SupervisorHomeScreen.kt`): nombre del trabajador clickeable (visible solo cuando "No, no todos trabajaron..."), ícono de lápiz (`Icons.Filled.Edit`) si ya tiene comentario, modal nuevo `ComentarioPersonaDialog` con Guardar/Borrar (Borrar solo visible si ya existía)/Cancelar.

**Bug real encontrado y corregido en verificación en vivo** (no solo en teoría): el comentario por trabajador reutiliza el mismo endpoint/payload que "Guardar registro" (`personas[].comment` viaja junto con `personas[].worked_hours`) — pero si el supervisor abre el modal y guarda un comentario **antes** de haber tocado "Guardar registro" alguna vez, `SupervisorActivityEntity.allWorkedScheduledHours` seguía en `null` en Room. `SupervisorSyncRepository.pushRegistrosPendientes()` usa `actividad.allWorkedScheduledHours ?: true` para decidir si manda la lista de `personas` — con `null`, caía en `true` y mandaba `personas = []`, así que el comentario se guardaba local pero **nunca llegaba al servidor**, sin ningún error visible. Se verificó consultando directamente la tabla `activity_employee_comments` en Postgres tras guardar desde la app real (0 filas) antes de encontrar la causa. Fix: `onGuardarComentarioPersona` ahora manda también el snapshot actual del formulario (`comments`/`todosTrabajaron`/`horasPorPersona`), y el ViewModel llama primero a `guardarRegistroLocal(...)` (persiste ese snapshot, igual que "Guardar registro") y luego a `guardarComentarioPersonaLocal(...)` — re-verificado en vivo: el comentario llegó, con `all_worked_scheduled_hours = false` y `reported_hours` correctos. De paso se encontró y arregló un problema cosmético: con los 3 botones del modal (Borrar/Cancelar/Guardar) el padding por defecto partía "Guardar" en dos líneas — se redujo `contentPadding` de los 3 botones.

**Archivos modificados (Laravel)**: `app/Http/Controllers/Api/Personal/ActivityController.php`, `app/Http/Controllers/Personal/BitacoraController.php`, `app/Models/Activity.php`, `resources/views/personal/bitacora/index.blade.php`, `tests/Feature/Api/Personal/ActivityControllerTest.php`, `tests/Feature/Personal/BitacoraControllerTest.php`. **Nuevos**: `app/Models/ActivityEmployeeComment.php`, `database/migrations/2026_09_22_100000_create_activity_employee_comments_table.php`, `database/migrations/2026_09_22_100001_migrate_bitacora_comments_to_history_table.php`.

**Archivos modificados (Android)**: `AppDatabase.kt`, `DatabaseProvider.kt`, `SupervisorMigrations.kt`, `SupervisorPersonaEntity.kt`, `SupervisorPersonaDao.kt`, `PersonalActivityDtos.kt`, `PersonalActivityLocalRepository.kt`, `SupervisorSyncRepository.kt`, `SupervisorActivityViewModel.kt`, `SupervisorHomeScreen.kt`, `MainActivity.kt`.

**Verificado en esta sesión**:
- Laravel: 4 tests nuevos en `ActivityControllerTest` (guardar/editar/borrar comentario del responsable, y que no se borra al marcar "Sí" por error) + 3 tests nuevos en `BitacoraControllerTest` (historial de admin se acumula, comentario vacío no crea fila, comentario del responsable aparece marcado en el historial) — todos pasando. Suite completa: 116/117 (mismo `ExampleTest` preexistente sin relación).
- Bitácora web verificada en vivo con Puppeteer contra el servidor local real: historial mezclado (responsable + 2 de admin) se ve correctamente en el modal, con las etiquetas correctas y en el orden en que se escribieron.
- Android: `compileDebugKotlin` + `testDebugUnitTest` + `assembleDebug` sin errores. Verificación en vivo end-to-end contra un backend real (Laravel local, no producción — ver nota abajo): login real como supervisor (`lfdo`), actividad de prueba desechable, guardar/editar/borrar un comentario de un trabajador desde la app, confirmado en cada paso contra la tabla `activity_employee_comments` real (no solo en pantalla). Se encontró y corrigió el bug de sincronización descrito arriba durante esta misma verificación. Datos de prueba (actividad, empleados, empresa) limpiados al terminar, con conteos antes/después verificados.
- **Nota importante**: la verificación Android se hizo apuntando `RetrofitClient.BASE_URL` temporalmente a `http://10.0.2.2:8000/` (backend local) en vez de producción, siguiendo el patrón ya documentado en ese mismo archivo — se revirtió a producción antes de terminar y se re-compiló/re-testeó para confirmar el estado final. **El backend de producción (Railway) todavía NO tiene esta migración ni este código desplegado** — el feature completo (Laravel + Android) está listo mergeado localmente pero pendiente de despliegue explícito.
